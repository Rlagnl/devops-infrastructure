...
EOF
    ...
    # ts-langchain-server 通过此网络用容器名 aui-components-mcp-server:3001 调用 MCP 工具
    docker network create app-network 2>/dev/null || echo "app-network 已存在, 跳过创建"
}

# ============ 2. 创建 compose 配置 ============
# 生产版 compose: 拉取 ghcr 镜像, 无 build 字段
# 单服务, 无需 postgres/redis
# 加入 app-network (供 ts-langchain-server 容器内调用)
step_2_create_compose() {
    mkdir -p /opt/aui-components-mcp-server
    cd /opt/aui-components-mcp-server

    # EOF 不加引号: ${GITHUB_USER} 需要被 shell 展开写入镜像地址
    # ${GITHUB_USER,,} 把用户名转小写(ghcr.io 镜像名必须全小写)
...
step_5_setup_runner() {
    ...
    # deploy job 以 github-runner 用户运行, 需要对项目目录有写权限(执行 docker compose)
    # /opt/aui-components-mcp-server 由 step_2 以 root 创建, 这里移交所有者
    chown -R github-runner:github-runner /opt/aui-components-mcp-server
...
step_6_verify() {
    ...
    if [[ -f /opt/aui-components-mcp-server/docker-compose.yml ]]; then
    ...
    # 检查 app-network
    if docker network ls --format '{{.Name}}' | grep -qx 'app-network'; then
        echo "[OK] app-network 已存在"
    else
        echo "[WARN] app-network 不存在, 请确认 step_1 已执行"
    fi
