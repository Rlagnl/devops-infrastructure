...
#   5. nginx 监听 8084,proxy_pass 127.0.0.1:2025,支持 SSE 长连接
#   6. 数据目录 /opt/langgraph-server/
#   7. 比 TS 版多 Supabase / DashScope / Cohere 相关 RAG 变量
#
...
step_6_verify() {
    ...
    echo ""
    echo "=== 后续步骤 ==="
    echo "1. 在 GitHub 仓库配置 Variables(非敏感) 与 Secrets(敏感), 见部署方案 4.2 节"
    ...
    echo "5. 部署完成后访问: http://<服务器IP>:8084/ok"
}
